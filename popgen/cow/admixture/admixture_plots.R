setwd("~/github_repos/snowtracks/popgen/cow/admixture")

library(tidyverse)

# === Load static data ===
fam  <- read.table("cows.fam", header = FALSE)
pops <- read.table("ena_metadata.tsv", header = FALSE, sep = "\t")
...
plot_admixture <- function(K) {
  ...
  # --- Load Q file ---
  Q <- read.table(paste0("cows.", K, ".Q"), header = FALSE)
  ...
  # --- Save plot ---
  ggsave(
    filename = paste0("cow_plot_K", K, ".png"),
    plot = p,
